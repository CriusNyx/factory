...
- [n/a] Fixing the equality operator by making parser less greedy.
  - Rejected. Don't want to rework the language grammar.
...
- [ ] Remove the limit keyword and change the grammar for the in keyword.
- [ ] Update the grammar for the out keyword to specify an output amount.
- [ ] Update invocation mapping to support method attributes.
- [ ] Remove dynamic argument binding and replace it with C# argument binding.
  - [ ] Update method binding to check against C# overloads instead of factory method overloads.
- [ ] Update recipe solver to use a constraint solver.
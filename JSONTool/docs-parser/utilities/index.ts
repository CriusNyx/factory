...
const materialRegex = new RegExp(
  `_(${Object.keys(MATERIAL_SLUGS).join("|")})(?:_|InCorner|OutCorner)`
);
...
export function createBuildableRecipeSlug(
  className: string,
  displayName: string
) {
...
export function standardizeItemDescriptor(className: string) {
  return className.replace(
    /^(?:BP_EquipmentDescriptor)|(?:BP_ItemDescriptor)|(?:BP_EqDesc)/,
    "Desc_"
  );
}
...
export function buildableNameToDescriptorName(buildableName: string) {
  ...
  // The regex returns a trailing " for some reason.
  return buildableName.replace(/^Build_/, "Desc_").replace('"', "");
}
...
export function parseItemQuantity(
  data: any,
  itemData: ParsedClassInfoMap<ItemInfo>
): ItemQuantity {
  const className = standardizeItemDescriptor(
    parseBlueprintClassname(data.ItemClass)
  );
...
export function parseBuildableQuantity(
  data: any,
  buildableData: ParsedClassInfoMap<BuildableInfo>
): string {
  const className = standardizeItemDescriptor(
    parseBlueprintClassname(data.ItemClass)
  );

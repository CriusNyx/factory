...
import {
  ItemQuantity,
  buildableNameToDescriptorName,
  createBuildableRecipeSlug,
  createCustomizerSlug,
  createRecipeSlug,
  getShortClassname,
  parseBuildableQuantity,
  parseCollection,
  parseItemQuantity,
} from "../utilities";
...
export function parseRecipes(
  categorizedDataClasses: CategorizedDataClasses,
  deps: RecipeDependencies
) {
  const { productionRecipes, buildableRecipes } = getMainRecipes(
    categorizedDataClasses,
    deps
  );
...
function getMainRecipes(
  categorizedDataClasses: CategorizedDataClasses,
  { items, buildables }: RecipeDependencies
) {
  ...
  categorizedDataClasses.recipes.forEach((entry) => {
    ...
    const producedIn = parseCollection<any[]>(entry.mProducedIn).map((data) =>
      getShortClassname(data)
    );
    ...
    for (const producer of producedIn) {
      if (producer === "BP_BuildGun_C" || producer === "FGBuildGun") {
        ...
        producer === "Build_AutomatedWorkBench_C"
      ) {
        handCraftable = true;
      } else {
    ...
    const ingredients = parseCollection<any[]>(entry.mIngredients).map((data) =>
      parseItemQuantity(data, items)
    );

    if (isBuildRecipe) {
      const product = parseBuildableQuantity(
        parseCollection<any[]>(entry.mProduct)[0],
        buildables
      );
      ...
      const isAlternate =
        entry.mDisplayName.startsWith("Alternate:") ||
        entry.ClassName.startsWith("Recipe_Alternate");
      const products = parseCollection<any[]>(entry.mProduct).map((data) =>
        parseItemQuantity(data, items)
      );
...
function getCustomizerRecipes(
  categorizedDataClasses: CategorizedDataClasses,
  { items }: RecipeDependencies
) {
  ...
  categorizedDataClasses.customizerRecipes.forEach((entry) => {
    ...
    if (entry.mIngerdients) {
      ingredients = parseCollection<any[]>(entry.mIngredients).map((data) =>
        parseItemQuantity(data, items)
      );
    } else {
...
function validateBuildableRecipes(
...
function validateCustomizerRecipes(
  ...
  Object.entries(customizerRecipes).forEach(([name, data]) => {
    if (!data.slug) {
      ...
      // eslint-disable-next-line no-console
      console.warn(
        `WARNING: Duplicate customizer recipe slug: [${data.slug}] of [${name}]`
      );
    } else {
      slugs.push(data.slug);
    }
